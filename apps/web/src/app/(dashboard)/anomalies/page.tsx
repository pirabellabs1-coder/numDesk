  ...
  return (
    <section className="mx-auto max-w-5xl space-y-8">
      <div className="flex items-end justify-between">
        <div>
          <h1 className="text-4xl font-bold tracking-tight text-on-surface" style={{ fontFamily: "Inter, sans-serif" }}>Détection d&apos;anomalies</h1>
      ...
      <div className="space-y-3">
        {anomalyList.map((anomaly: any) => {
          ...
          return (
            <div key={anomaly.id} className={`rounded-2xl border p-5 transition-all ${anomaly.resolved ? "border-white/5 bg-card opacity-60" : "border-error/10 bg-card"}`}>
              <div className="flex items-start gap-4">
                <div className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-xl ${sev.bg}`}>
                  <span className={`material-symbols-outlined ${sev.color}`}>{typeIcons[anomaly.type] || "warning"}</span>
                </div>
                <div className="min-w-0 flex-1">
                  ...
                  <p className="mt-1 text-sm text-on-surface-variant">{anomaly.description}</p>
                  <p className="mt-2 text-[11px] text-on-surface-variant">{new Date(anomaly.detectedAt).toLocaleString("fr-FR")}</p>
                </div>
                {!anomaly.resolved && (
                  <button onClick={() => handleResolve(anomaly.id)} disabled={resolveAnomaly.isPending} className="shrink-0 rounded-lg border border-white/10 px-3 py-1.5 text-xs font-bold text-on-surface-variant hover:text-on-surface disabled:opacity-50">
